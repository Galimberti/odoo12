...
class RestApi(http.Controller):
    ...
    @http.route(['/start'], \
        type="http", auth="public", csrf=False, website=True, cors='*')
    ...
    @http.route(['/restapi/1.0/common/oauth2/authorize'], \
        type="http", auth="user", csrf=True, website=True, cors='*')
    ...
    def auth_access(self, **kwargs):
        ...
        auth_auth = obj.search([('consumer_key', '=', request.httprequest.authorization.username),
                                ('consumer_secret', '=', request.httprequest.authorization.password)])
        if not auth_auth:
            return self.get_response(401, '401', {"code":401, "message": "Invalid Credentials."})
        ...
        if data:
            def myconverter(o):
                if isinstance(o, datetime):
                    return o.__str__()
            try:
                response.data = isinstance(data, str) and data or json.dumps(data, default=myconverter)
    ...
    @http.route(['/restapi/1.0/common/oauth1/authorize'], \
        type="http", auth="user", csrf=False, website=True, cors='*')
    ...
    def valid_authentication(self, kwargs):
        ...
        if kwargs.get('oauth_signature'):
            ...
            if not access_token or access_token.access_token_validity < datetime.now():
